...
/**
 * TASK 3: Challenge Plan Local Caching
 * 
 * Cache the daily MCQ challenge plan in localStorage to avoid redundant API calls.
 * The plan doesn't change during the day, so we cache it once and reuse it for all page navigations.
 *
 * Performance: 1 API read per day per user instead of 1 per page navigation (5-10x reduction)
 * 
 * Firestore impact: Eliminates repeated getDoc calls for dailyPlans collection
 */

const DAILY_PLAN_CACHE_KEY = "cee-daily-plan-cache";
const DAILY_PLAN_CACHE_VERSION = 4;

/**
 * Get cached daily plan if it exists and belongs to today
 * Returns null if cache is missing or stale (from yesterday)
 */
export function getCachedDailyPlan(dateKey = getDailyDateKey()) {
  if (typeof window === "undefined") return null;

  try {
    const cached = window.localStorage.getItem(DAILY_PLAN_CACHE_KEY);
    ...
    if (
      ...
      parsed.plan &&
      parsed.plan.promptVersion === DAILY_PLAN_CACHE_VERSION
    ) {
      console.log(`✅ Using cached daily plan for ${dateKey}`);
      return parsed.plan;
    }

    // Cache is from yesterday or older - discard it
    return null;
  } catch (error) {
    console.warn("⚠️ Failed to read daily plan cache:", error);
    return null;
  }
}

/**
 * Cache the daily plan locally after fetching from API
 * This prevents redundant API calls during the same day
 */
export function cacheDailyPlan(plan, dateKey = getDailyDateKey()) {
  if (typeof window === "undefined" || !plan) return;

  try {
    window.localStorage.setItem(
      DAILY_PLAN_CACHE_KEY,
      JSON.stringify({
        dateKey,
        plan,
        promptVersion: DAILY_PLAN_CACHE_VERSION,
        cachedAt: new Date().toISOString(),
      })
    );
    console.log(`💾 Cached daily plan for ${dateKey}`);
  } catch (error) {
    console.warn("⚠️ Failed to cache daily plan:", error);
  }
}

// Daily progress is stored per-user in Firestore under users/{userId}
// Use the storage.js version: readDailyProgress, writeDailyProgress
...
export async function incrementStudyStreakIfNeeded(
  ...
  if (!userId) {
    console.warn("⚠️ incrementStudyStreakIfNeeded: Missing userId. Streak will not be saved.");
  ...
  try {
    ...
    console.warn("⚠️ Failed to fetch remote streak, using local:", error?.message);
  }
  ...
  if (current.lastDate === dateKey) {
    console.log(`🔄 Streak already updated for ${dateKey}`);
    return current;
  }
  ...
  console.log(`🔥 Updating streak: ${current.count} → ${nextCount} for user ${userId}`);
  
  writeStudyStreakLocal(nextState.count, nextState.lastDate);
  console.log("✅ Streak updated locally.");
  
  try {
    await writeStudyStreakRemote(nextState.count, nextState.lastDate, userId);
    console.log("✅ Streak updated remotely.");
  } catch (error) {
    console.error("❌ Failed to update streak remotely:", error?.message || error);
    throw error;
  }
