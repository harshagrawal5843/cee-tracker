...
export function incrementStudyStreakIfNeeded(
  dateKey = getDailyDateKey(),
  userId = null,
) {
  const current = readStudyStreak();
  ...
  writeStudyStreak(nextState.count, nextState.lastDate, userId);
  return nextState;
}
...
export async function migrateAndSyncTimerData(userId) {
  ...
  const localState = readStudyTimerState();

  let mergedHistory = { ...(remoteData?.history || {}) };
  let historyChanged = false;

  // Merge history: keep the maximum time for each date (this preserves the last 2-3 days of local data)
  ...
  // If local had newer/longer times, push the updated history to Firebase
  if (historyChanged) {
  ...
  // Push current local state to remote
  await writeStudyTimerStateRemote(localState, userId);
}
