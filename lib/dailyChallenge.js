...
export async function migrateAndSyncTimerData(userId) {
  ...
  // Merge history (keep highest values)
  ...
  // Convert old local seconds into V2 Timestamps so running timers don't stop
  const newState = {
    version: "v2",
  ...
  // Clear old local active flag to prevent re-migration
  if (typeof window !== "undefined") {
    window.localStorage.setItem(STORAGE_SESSION_ACTIVE, "false"); 
  }
}
