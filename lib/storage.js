...
export async function writeDailyProgress(
  ...
  if (!userId || !dateKey) {
    console.warn(
      "writeDailyProgress: Missing userId or dateKey",
      { userId, dateKey }
    );
    return;
  }
  ...
  try {
    if (!firebaseAvailable) {
      throw new Error(
        "Firebase not available. Check configuration and ensure you have internet."
      );
    }
    ...
    console.log(
      `✅ writeDailyProgress: Saved for ${userId} on ${dateKey}`
    );
  } catch (error) {
    console.error("❌ Error writing daily progress to Firebase:", error?.message || error);
    throw error;
  }
}
...
export async function readStudyStreakRemote(userId = null) {
  if (!firebaseAvailable || !userId) {
    console.warn("⚠️ readStudyStreakRemote: Firebase not available or userId missing");
    return null;
  }
  try {
    const userRef = doc(db, "users", userId);
    ...
    if (!snap.exists()) {
      console.warn(`⚠️ readStudyStreakRemote: No user document found for ${userId}`);
      return { count: 0, lastDate: null };
    }
    const data = snap.data();
    const streak = data?.studyStreak || { count: 0, lastDate: null };
    console.log(`✅ readStudyStreakRemote: Retrieved streak for ${userId}:`, streak);
    return streak;
  } catch (error) {
    console.error("❌ readStudyStreakRemote failed:", error?.message || error);
    throw error;
  }
}
...
export async function writeStudyStreakRemote(count, lastDate, userId = null) {
  if (!firebaseAvailable || !userId) {
    console.warn("⚠️ writeStudyStreakRemote: Firebase not available or userId missing");
    return;
  }
  try {
    const userRef = doc(db, "users", userId);
    ...
    console.log(`✅ writeStudyStreakRemote: Saved streak ${count} for user ${userId}`);
  } catch (error) {
    console.error("❌ writeStudyStreakRemote failed:", error?.message || error);
    throw error;
  }
}

export async function writeDailyChallengeHistory(record, userId = null) {
  if (!record?.date || !userId) {
    console.warn(
      "writeDailyChallengeHistory: Missing record.date or userId",
      { date: record?.date, userId }
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `✅ writeDailyChallengeHistory: Saved for ${userId} on ${record.date}`
    );
  } catch (error) {
    console.error("❌ Error writing daily challenge history to Firebase:", error?.message || error);
    throw error;
  }
}
...
// ADD TO THE BOTTOM OF lib/storage.js

export async function readStudyTimerDataRemote(userId) {
  ...
  try {
    ...
    const data = snap.data();
    return {
      state: data.studyTimerState || null,
      history: data.studyTimerHistory || {}
    };
  } catch (error) {
    console.warn("Error reading timer remote:", error);
    return null;
  }
}

export function subscribeToStudyTimerState(userId, callback) {
  ...
  const unsubscribe = onSnapshot(userRef, (docSnap) => {
    if (docSnap.exists()) {
      const data = docSnap.data();
      callback(data.studyTimerState || null);
    } else {
      callback(null);
    }
  }, (error) => {
    console.warn("Timer sync error:", error);
  });
  
  return unsubscribe;
}

export async function writeStudyTimerStateRemote(state, userId) {
  if (!firebaseAvailable || !userId) return;
  try {
    const userRef = doc(db, "users", userId);
    await setDoc(userRef, { studyTimerState: state, lastUpdated: new Date() }, { merge: true });
  } catch (error) {
    console.warn("Error writing timer state remote:", error);
  }
}

export async function writeStudyTimerHistoryRemote(historyMap, userId) {
  if (!firebaseAvailable || !userId) return;
  try {
    const userRef = doc(db, "users", userId);
    await setDoc(userRef, { studyTimerHistory: historyMap, lastUpdated: new Date() }, { merge: true });
  } catch (error) {
    console.warn("Error writing timer history remote:", error);
  }
}

/**
 * TASK 2: Batched Write for Quiz Completion
 * 
 * REPLACES 3 separate writes with 1 single operation:
 *   ❌ OLD: writeDailyProgress() + writeStudyStreakRemote() + writeDailyChallengeHistory() = 3 writes
 *   ✅ NEW: saveQuizCompletionData() = 1 write
 *
 * Performance Impact: 66% reduction in write operations for quiz completion
 * 
 * @param {string} userId - User ID for data persistence
 * @param {string} dateKey - Date key (YYYY-MM-DD format)
 * @param {Object} progressUpdate - { "Subject::Chapter": taskResult }
 * @param {Object} historyRecord - { date, plan, tasks, source, ... }
 * @param {Object} streakUpdate - { count, lastDate } for streak increment
 * @returns {Promise<void>}
 */
export async function saveQuizCompletionData(
  userId,
  dateKey,
  progressUpdate,
  historyRecord,
  ...
  if (!userId || !dateKey) {
    console.error(
      "❌ saveQuizCompletionData: Missing userId or dateKey",
      { userId, dateKey }
    );
    throw new Error("Invalid userId or dateKey");
  }

  if (!firebaseAvailable) {
    console.error("❌ Firebase not available");
    throw new Error("Firebase not available. Check configuration.");
  }

  try {
    // Read current document once
    const userRef = doc(db, "users", userId);
    const docSnap = await getDoc(userRef);
    const currentData = docSnap.exists() ? docSnap.data() || {} : {};

    // Merge all updates into single payload
    const payload = {
      ...
      // Update challenge history for today
      dailyChallengeHistoryByDate: {
        ...(currentData.dailyChallengeHistoryByDate || {}),
        [dateKey]: {
          ...historyRecord,
          updatedAt: new Date().toISOString(),
        },
      },
      // Update streak if provided (only when all tasks completed)
      studyStreak: streakUpdate || (currentData.studyStreak || { count: 0, lastDate: null }),
      lastUpdated: new Date(),
    };

    // SINGLE WRITE: All updates in one operation
    await setDoc(userRef, payload, { merge: true });

    console.log(
      `✅ saveQuizCompletionData: Completed all updates in 1 write for ${userId} on ${dateKey}`
    );
    return payload;
  } catch (error) {
    console.error(
      "❌ saveQuizCompletionData failed:",
      error?.message || error
    );
    throw error;
  }
